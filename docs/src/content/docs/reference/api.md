...
Only `AuthorizationPlan.Value` reaches the handler, and only after an `Allow`
decision. `HEAD` uses the matching GET route and runs its authorization phase.
`OPTIONS` is generated by the framework and returns `204` with `Allow`.
